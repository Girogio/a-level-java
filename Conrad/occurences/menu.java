...
class menu {
    ...
    public static void main(String args[]) {
        ...
        while (true) {
            ...
            int[] out = check.result(query, input);

            for(int i = 0; i < out.length; i++){

                if(out[i] == 0){
                    System.out.println(avg.calcAverage(check.result(query, input), true));
                    break;
                }else if (i == out.length -1){
                    System.out.println(avg.calcAverage(check.result(query, input)));
                }
            }
